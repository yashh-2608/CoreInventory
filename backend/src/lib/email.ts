...
const emailHtml = (otp: string) => `
  <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 40px; border: 1px solid #e2e8f0; border-radius: 24px; background-color: #020617; color: white;">
    ...
    <div style="margin-top: 40px; padding-top: 24px; border-top: 1px solid #1e293b; text-align: center;">
      <p style="color: #475569; font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em;">&copy; 2026 CoreInventory SaaS System</p>
    </div>
  </div>
`;

const sendViaBrevo = async (email: string, otp: string) => {
  const payload = {
...
export const sendOtpEmail = async (email: string, otp: string) => {
  try {
    ...
    console.warn(`OTP email transport not configured. OTP for ${email}: ${otp}`);
  } catch (error) {
